...
wav_type = '.pcm'
wav_path = ''  # 스맛폰에서 보내온 녹음 파일 경로 (pcm->wav)
wav_name = ''  # 스맛폰에서 보내온 녹음 파일 이름 (pcm->wav)
img_type = '.png'
img_path = ''  # 녹음 파일을 mel spectrogram 으로 변환하여 저장할 경로
...
def pcm_to_wav():
    command = "ffmpeg -i " + wav_path + wav_name + wav_type + \
